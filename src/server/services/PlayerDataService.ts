...
export default class PlayerDataService implements OnInit, OnStart {
	...
	*/
	...
	public onStart() {
		const createProfileForPlayer = async (player: Player) => {
			try {
				debug(`[${script.Name}] PlayerAdded: ${player.Name} -> loading profile`);
				await this.load(player);
				info(`[${script.Name}] loaded ${player.Name}'s profile`);
			} catch (e) {
				warn(`[${script.Name}] failed loading profile for ${player.Name}: ${e}`);
			}
		};
	...
	private async load(player: Player): Promise<Types.PlayerProfile> {
		...
		const promise = (async (): Promise<Types.PlayerProfile> => {
			...
			if (!profile) {
				...
				throw `[${script.Name}] could not load profile for ${player.Name} after ${this.MAX_RETRIES} tries`;
			}

			if (!profile.Data || !typeIs(profile.Data, "table")) {
				warn(`[${script.Name}] invalid profile data for ${player.Name}`);
				profile.EndSession();
				throw `[${script.Name}] invalid profile data for ${player.Name}`;
			}
			...
			trove.add(plrSignals.SurvivalsChanged);
	...
	public async GetPlayerData(player: Player): Promise<Types.PlayerData> {
		...
		if (!profile) {
			...
			throw `[${script.Name}] profile not loaded for ${player.Name}`;
		}
	...
	public async SetPlayerStat(player: Player, statName: Types.ValidStats, value: number): Promise<boolean> {
		...
		if (!typeIs(sig, "table")) {
			...
			throw `[${script.Name}] signals not initialized for ${player.Name}`;
		}
	...
	public async ClearPlayerData(player: Player): Promise<boolean | unknown> {
		if (!RunService.IsStudio()) {
			...
			throw `[${script.Name}] ClearPlayerData can only be used in Studio`;
		}

		assert(this.playerCheck(player));
		info(`[${script.Name}] ClearPlayerData: resetting stats for ${player.Name}`);

		await Promise.all([this.SetPlayerStat(player, "Survivals", 0), this.SetPlayerStat(player, "Points", 0)]);
